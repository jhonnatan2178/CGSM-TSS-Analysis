...
\begin{figure}[h!]
    ...
    (e)~Diverging long-term trends (S1 water area vs.\ reflectance):
    pending Sentinel-1 SAR water-area time series, not generated this
    session. (f)~Band--TSS Pearson correlation matrix; Sentinel-2
    values use the atmospherically corrected reflectance
    (Section~\ref{sec:disc_s2gap}), which differs substantially from
    the pre-correction values ($r \approx 0.08$--$0.12$) shown in
    earlier analyses of this dataset.}
...
\citep{Blanco2006}. This interpretation should, however, be treated
with caution: both S1 water area and reflectance/TSS show significant
monotonic trends over the same 2015--2025 period
(Mann--Kendall $\tau = +0.18$ for S1 area, $\tau = -0.27$ to $-0.31$
for reflectance), and with only $n = 10$ annual points, a bivariate
Pearson or Spearman correlation between two co-trending series cannot,
on its own, distinguish a genuine dilution relationship from a shared,
possibly independent long-term drift in each series (e.g.\ a regional
hydroclimatic shift affecting both variables without either causing
the other). Disentangling these requires either detrending both series
prior to correlation (e.g.\ correlating first differences or
residuals from each series' own linear trend) or a partial correlation
controlling for time directly; neither was performed for this
manuscript draft, since it requires re-analysis of the underlying
annual S1 area and TSS series rather than the summary statistics
reported here, and is flagged explicitly as an open item in
Section~\ref{sec:disc_limits} rather than presented as resolved. The
diverging trends --- S1 area increasing while
...
changing catchment land use; this trend-level interpretation does not
depend on the bivariate correlation and is comparatively more robust
to the confound described above, since it compares the sign and
significance of two independent Mann--Kendall trend tests rather than
correlating the two series against each other directly. Both patterns
are consistent with
...
correlation reported in Section~\ref{sec:results_hydro} is subject to
the shared-trend confound discussed in
Section~\ref{sec:disc_hydro}: with only $n=10$ annual points and both
series exhibiting significant independent long-term trends, a
detrended or partial correlation -- not performed in this manuscript
draft -- is needed before the dilution-mechanism interpretation can be
considered confirmed rather than consistent with the data. The S1 hydroclimatic
...
\begin{enumerate}
    ...
    correlation ($r = -0.766$, $p = 0.010$, $n=10$) is consistent with
    a freshwater-dilution mechanism but should be interpreted with
    caution, since both series exhibit significant independent
    long-term trends over the same period and a bivariate correlation
    between two co-trending series with only ten annual points cannot,
    on its own, rule out a shared, possibly unrelated drift in each
    series as the source of the association; a detrended or partial
    correlation is needed to confirm the dilution interpretation
    specifically (Section~\ref{sec:disc_hydro}).
