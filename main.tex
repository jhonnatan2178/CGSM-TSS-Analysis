...
centred on the sampling date. To determine the optimal temporal range, a sensitivity analysis was conducted by progressively expanding the matchup window from 0 (same-day) up to $\pm$3~days, evaluating cross-validated model performance at each threshold
(Section~\ref{sec:sensitivity}).

Model performance improved consistently up to $\pm$2.5~days and
plateaued between $\pm$2.5 and $\pm$3.0~days, indicating temporal
de-correlation of TSS dynamics at longer lags and diminishing returns
from additional matchup pairs. Based on this analysis, a maximum window
of $\pm$2.5~days was selected, balancing temporal representativeness
and dataset robustness. All matchups in the final dataset fall within
this range (maximum observed $|\Delta t| = 2.84$~days).
...
Figure~\ref{fig:sensitivity} shows CV~$R^2$ as a function of the
maximum temporal lag threshold. Performance plateaued at $\pm$2.5~days,
confirming the selected window.
 
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.80\linewidth]{images/fig1_temporal_sensitivity.png}
    \caption{CV~$R^2$ as a function of maximum temporal
    matchup window (Pajarales L8, B4, fixed $C_p$ NECHAD). Error bars:
    fold-to-fold standard deviation. Performance plateaus at
    $\pm$2.5~days.}
...
sensitivity analysis confirmed $\pm$2.5~days as the optimal window, with
no significant relationship between temporal lag and absolute residual
($r = -0.20$, $p = 0.08$), validating the temporal weighting strategy.
